...
export const getReports = async (req, res) => {
  ...
  const [invoices, purchases, medicines, batches, customers, suppliers, discounts, returns] =
    await Promise.all([
  ...
  const totalSales = invoices.reduce((sum, sale) => sum + sale.grandTotal, 0);
  ...
  res.json({
    cards: [
      ...
      { title: "GST report", value: invoices.reduce((sum, sale) => sum + sale.gstTotal, 0).toFixed(2) },
    ...
    salesTable: invoices.slice(0, 10).map((invoice) => ({
      ...invoice.toObject(),
